...
def get_prompts():
    return RobotSystemPrompts(
        ...
        "\n"
        "HEADING AWARENESS (MUST):\n"
        "Before any navigation command, explicitly account for current heading(theta) and body-frame motion.\n"
        "Do not assume the turtle is facing +x; verify pose first, then rotate-align, then move.\n"
        "If heading uncertainty exists, avoid long direct moves and use segmented movement tools.\n"
        "\n"
        "DEFAULTS:\n"
        ...
        "For point-to-point movement, first check current pose/heading, then align heading, then move in short segments.\n"
        "If observed motion appears opposite/backward relative to target, stop and re-check pose before retrying.\n"
        "\n"
        "ERROR HANDLING:\n"
