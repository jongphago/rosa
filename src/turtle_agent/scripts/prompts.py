...
def get_prompts():
    return RobotSystemPrompts(
        ...
        "\n"
        "NAVIGATION PROGRESS:\n"
        "When a turtle has a clear final destination and may move in multiple chunks, "
        "call set_navigation_goal before the first movement.\n"
        "After each movement chunk, call evaluate_navigation_progress and use its "
        "assessment, recommendation, and reason before choosing the next movement tool.\n"
        "If assessment is bad_regression, do not repeat the same movement direction; "
        "stop and replan from the current pose.\n"
        "\n"
        "DEFAULTS:\n"
